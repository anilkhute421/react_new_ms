...
const schema = yup.object().shape({
  ...
  conatctNumber: yup.string().required('Contact number is required'),
...
function EditEmployee(props) {
  ...
  const emp_edit_id = useLocation().state?.project_id;
  // console.log(emp_edit_id, 'emp_id');

  const { employee_view } = useSelector((state) => state?.employee);
  console.log(employee_view, 'employee_viewgjghj');

  const { handleSubmit, control, register, setValue, watch } = useForm({
    resolver: yupResolver(schema)
  });
  const { editEmployee } = useEmployeeHook();
  ...
  useEffect(() => {
    // setValue('employeeId', employee_view?.id);
    setValue('old_id', employee_view?.old_id);
    setValue('dob', employee_view?.date_of_birth);
    setValue('firstName', employee_view?.first_name);
    setValue('lastName', employee_view?.last_name);
    setValue('gender', employee_view?.gender);
    setValue('bloodGroup', employee_view?.blood_group);
    setValue('maritalStatus', employee_view?.martial_status);
    setValue('conatctNumber', employee_view?.contact_number);
    setValue('emergencyCN', employee_view?.emergency_contact_number);
    setValue('whatsappNumber', employee_view?.whatsapp_number);
    setValue('emailPersonal', employee_view?.email);
    setValue('udid', employee_view?.udid);
    setValue('panNo', employee_view?.pan_no);
    setValue('caddress1', employee_view?.currentAddress?.address_line_one);
    setValue('caddress2', employee_view?.currentAddress?.address_line_two);
    setValue('ccity', employee_view?.currentAddress?.city);
    setValue('cpostcode', employee_view?.currentAddress?.postalcode);
    setValue('cstate', employee_view?.currentAddress?.state);
    setValue('paddress1', employee_view?.permanentAddress?.address_line_one);
    setValue('paddress2', employee_view?.permanentAddress?.address_line_two);
    setValue('pcity', employee_view?.permanentAddress?.city);
    setValue('ppostcode', employee_view?.permanentAddress?.postalcode);
    setValue('pstate', employee_view?.permanentAddress?.state);
  ...
  const requiredids = {
    ...
    cureent_add_id: employee_view?.currentAddress?.id,
    permanent_add_id: employee_view?.permanentAddress?.id
  };
  ...
  const onSubmit = (data, imageName) => {
    ...
    if (dateObj > currentDate) {
      ...
      editEmployee(data, imageName, requiredids, props.open());
    }
  };
                  ...
                  render={({ fieldState: { error }, field: { onChange, value } }) => (
                    <>
                      <CustomSelect
                        ...
                        onChange={onChange}
                        defaultValue={employee_view?.gender}
                      />
                    </>
                  )}
                />
                  ...
                  render={({ fieldState: { error }, field: { onChange, value } }) => (
                    <>
                      <CustomSelect
                        ...
                        onChange={onChange}
                        defaultValue={employee_view?.blood_group}
                      />
                    </>
                  )}
                />
                  ...
                  render={({ fieldState: { error }, field: { onChange, value } }) => (
                    <>
                      <CustomSelect
                        ...
                        onChange={onChange}
                        defaultValue={employee_view?.martial_status}
                      />
                    </>
                  )}
                />
                  ...
                  render={({ fieldState: { error }, field: { onChange, value } }) => (
                    <>
                      <CustomSelect
                        width="100%"
                        value={
                          employee_view?.currentAddress?.country
                            ? employee_view?.currentAddress?.country
                            : value
                        }
                        error={!!error}
