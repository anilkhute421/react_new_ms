...
function EditProject(props) {
  const projetcEditId = props;
  const { handleSubmit, control, register, setValue } = useForm({
    resolver: yupResolver(schema)
  });
  const { EditProject } = useProjectHook();

  const { drop_down } = useSelector((e) => e.signIn);

  const { project_view } = useSelector((state) => state.project);

  const endDatekey = new Date(project_view?.end_date).toISOString().split('T')[0];
  const startDatekey = new Date(project_view?.start_date).toISOString().split('T')[0];
  //changeing select option data(value to key).
  const projectTypeKey = project_view?.project_type === 'Fixed' ? '2' : '1';
  const projectConKey = project_view?.project_condition === 'New' ? '1' : '2';
  const projectPlateformKey =
    project_view?.platform === 'iOS'
      ? '1'
      : project_view?.platform === 'Android'
      ? '2'
      : project_view?.platform === 'Web'
      ? '3'
      : project_view?.platform === 'All'
      ? '4'
      : '';
  const sourcedKey =
    project_view?.sourced_from === 'upwork'
      ? '1'
      : project_view?.sourced_from === 'local'
      ? '2'
      : project_view?.sourced_from === 'guru'
      ? '3'
      : '';

  useEffect(() => {
    setValue('projectName', project_view?.name);
    setValue('clientName', project_view?.client_name);
    setValue('startDate', startDatekey);
    setValue('endDate', endDatekey);
    setValue('projectUrl', project_view?.url);
    setValue('noOfHours', project_view?.no_of_hours);
    setValue('budget', project_view?.budget);
    setValue('appName', project_view?.app_name);
    setValue('upworkId', project_view?.hired_id);
    setValue('notes', project_view?.notes);
    setValue('projectType', projectTypeKey);
    setValue('projectPlatform', projectPlateformKey);
    setValue('projectPhase', project_view?.project_phase);
    setValue('projectCondition', projectConKey);
  }, [project_view, setValue]);
  ...
  const onSubmit = (data) => {
    ...
    if (data?.projectUrl && !isValidUrl) {
      ...
      EditProject(data, props.setOpen(), projetcEditId?.projetcEditId);
    }
  };
    ...
    <>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <Box sx={{ padding: '30px', paddingBottom: '6rem' }}>
          <CustomText variant={'body_2'}>Edit Project</CustomText>
          <Grid rowSpacing={1} sx={{ mt: '25px' }} columnSpacing={'20px'} container>
            <Grid sx={{ marginBottom: '22px' }} item xs={6}>
              <FormControl fullWidth>
                <div style={{ display: 'flex', position: 'relative' }}>
                  {textC('Project Name')}
                  <label style={{ position: 'absolute', top: '0px', left: '99px', color: 'red' }}>
                    *
                  </label>
                </div>
                <Controller
                  ...
                  render={({ fieldState: { error }, field: { onChange, value } }) => (
                    <>
                      <CustomSelect
                        ...
                        onChange={onChange}
                        defaultValue={project_view?.project_phase}
                      />
                    </>
                  )}
                />
                  ...
                  render={({ fieldState: { error }, field: { onChange, value } }) => (
                    <>
                      <CustomSelect
                        ...
                        onChange={onChange}
                        defaultValue={projectPlateformKey}
                      />
                    </>
                  )}
                />
                    ...
                    <Controller
                      ...
                      render={({ fieldState: { error } }) => (
                        <FormFieldStyle
                          ...
                          type={field.type}
                          helperText={error ? error.message : null}
                          inputProps={{ min: field.type === 'number' ? 0 : undefined }}
                          placeholder="Enter"
...
EditProject.propTypes = {
  ...
  projetcEditId: PropTypes.string
};
