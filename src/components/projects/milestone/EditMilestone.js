...
import { editMilstoneAction, getAssigneeAction } from '../../../redux/project/action';
...
const EditMilestone = (props) => {
  const { setOpen, milesone_view, milestonelistingId } = props;

  // console.log(milestonelistingId, 'milestonelistingIdnewwww');
  // useEffect(() => {
  //   dispatch(viewMilestoneAction(milestonelistingId));
  // }, []);
  ...
  const selectedValue = milesone_view?.employee?.map((employee) => ({
  ...
  const { handleSubmit, control, register, setValue } = useForm({
    ...
    defaultValues: {
      links: milesone_view?.milestone_links
    }
  });
  ...
  // console.log(employee_drop_down_list, 'employee_drop_down_list');
  ...
  useEffect(() => {
    if (Object.keys(milesone_view).length > 0) {
      setValue('title', milesone_view?.name);
      setValue('milestone_detail', milesone_view?.milestone_detail);
      setValue('priorityLabel', PriorityKey);
      setValue('endDate', milesone_view?.due_date);
      setValue('startDate', milesone_view?.start_date);
    }
  }, [milesone_view, setValue]);
  ...
  useEffect(() => {
    dispatch(getAssigneeAction(project_id));
  }, []);

  const { project_view } = useSelector((state) => state.project);

  // const milestone = [
  //   { value: 'anil kk', key: 'SFS8540' },
  //   { value: 'anil kk', key: 'SFS6692' },
  //   { value: 'anil kk', key: 'SFS2074' }
  // ];
    ...
    <Card sx={{ overflowY: 'scroll' }}>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <Grid sx={{ padding: '30px' }} rowSpacing={1} columnSpacing={'20px'} container>
          <Box sx={{ padding: '20px' }}>
            <CustomText variant="body_2">Edit Milestone</CustomText>
          </Box>
          <GridItem item xs={12}>
          ...
          <GridItem item xs={12}>
            <FormControl fullWidth>
              ...
              <Controller
                ...
                control={control}
                rules={{
                  required: `Please select at least one Assignee`
                }}
                render={({ fieldState: { error }, field: { onChange } }) => (
                  <MultiSelect
                    name="assignee"
              ...
              <GridItem item xs={3}>
                ...
                <FormControl fullWidth>
                  ...
                  {/* <Controller
                    ...
                    render={({ fieldState: { error }, field: { onChange } }) => (
                      <CustomTextField
                        register={{ ...register(`links.${index}.link_title`) }}
                        label=""
                        type={'text'}
                        onChange={onChange}
                        error={!!error}
...
EditMilestone.propTypes = {
  ...
  milestonelistingId: PropTypes.string,
  milesone_view: PropTypes.object
};
